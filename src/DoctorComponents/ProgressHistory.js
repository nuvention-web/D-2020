        ...
        return (
            <div className={classes.root}>
                <Container>
                    ...
                    <div >
                        <ScatterChart className={classes.progressChart} width={600} height={400} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                            <CartesianGrid />
                            {/* tickFormatter={timeStr => moment(timeStr).format('HH:mm')} */}
                            <XAxis scale="time" type="number" domain={['dataMin', 'dataMax']} dataKey={'time'}
                                name='time' tickFormatter={timeStr => new Date(timeStr).toString().slice(0, 15)} 
                                label="Day"/>
                            <YAxis type="number" domain={[0, 5]} interval="0" dataKey={'pain'} name='pain' label="Pain Level"/>
                            <ZAxis range={[100]} />
                            ...
                            {Object.entries(historyData).map((d, i) => (
                                ...
                                <Scatter name={d[0]} data={d[1]} fill={'#' + Math.random().toString(16).substr(-6)} line shape="cross" />
                            ))}
